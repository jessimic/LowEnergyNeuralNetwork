...
for a_file in event_file_names:
    ...
    print("Total track events saved: %i, Total cascade events saved: %i, Saved events this file: %i, Finsihed file: %s"%(sum(count_energy_track),sum(count_energy_cascade),sum(keep_index),a_file))
    ...
    if np.all(count_energy_track >= max_per_bin) and np.all(count_energy_cascade >= max_per_bin):
