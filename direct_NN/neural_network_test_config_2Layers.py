...
parser.add_argument("--old_file", default=None,
                    dest="old_file", help = "filename of previous configurations and scores")
...
save = args.save
best_file_so_far = args.old_file
...
# Read in file
if best_file_so_far:
    old_configs, old_scores = numpy.genfromtxt(best_file_so_far, 'r')
    best_score = min(old_scores)
    index_best_score = old_scores.index(best_score)
    best_nodes = old_configs[index_best_score]
    old_score = True
else:
    old_score = False
...
save_nodes = []
save_score = []
save_within_diff = 1.0 #Score range within to keep
max_check = 10 #Ending 2**? value 
for n1 in range(3,max_check):
    for n2 in range(3,max_check):
        nodes = [2**n1, 2**n2]
        if nodes in old_configs:
            # If this was already checked and saved, skip it
            continue
        ...
        if len(save_score)==1 and old_score==False:
            #This is the first run, so initialize "best" variables
            best_score = new_score
            best_dt = new_dt
            best_model = new_model
            best_nodes = nodes
            print("No old score file to pull from, so setting new best")
        ...
        elif (abs(new_score-best_score)<save_within_diff and sum(nodes)<sum(best_nodes)):
