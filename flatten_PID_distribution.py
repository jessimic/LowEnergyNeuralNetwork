...
count_energy_track = np.zeros((bins))
count_energy_cascade = np.zeros((bins))
"""
...
"""
max_events_per_bin = [60000., 60000.,  29428.,  36859.,  44383.,  50909.,  58081.,
        20698.,  23499.,  25859.,  28204.,  30383.,  33022.,  35402.,
        37440.,  30181.,  31250.,  32011.,  33252.,  33841.,  34251.,
        35376.,  35631.,  36601.,  36869.,  36862.,  37533.,  37972.,
        38166.,  38525.,  38368.,  38876.,  39309.,  39621.,  39464.,
        34664.,  34423.,  34635.,  34687.,  34829.,  34322.,  34565.,
        34449.,  34614.,  34839.,  34728.,  34358.,  34157.,  34388.,
        34158.,  34169.,  33930.,  33562.,  33825.,  33724.,  33387.,
        33283.,  33446.,  33239.,  32618.,  33203.,  32715.,  32400.,
        32238.,  32182.,  32032.,  31886.,  31737.,  31285.,  31617.,
        31452.,  30980.,  30930.,  30718.,  30921.,  24663.,  24500.,
        24670.,  24526.,  24117.,  24126.,  24070.,  23610.,  24091.,
        23995.,  23608.,  23505.,  23479.,  23615.,  23192.,  23284.,
        23273.,  22872.,  22751.,  22892.,  22351.,  22474.,  22542.,
        22262.,  22417.,  22306.,  21959.,  21964.,  22030.,  21725.,
        21702.,  21534.,  21549.,  21606.,  21358.,  21275.,  20893.,
        21185.,  21116.,  21032.,  20919.,  21018.,  20630.,  20453.,
        20372.,  20620.,  20516.,  20469.,  20425.,  20331.,  20171.,
        20097.,  20156.,  19810.,  19655.,  19730.,  19981.,  19798.,
        19645.,  19456.,  19564.,  19545.,  19247.,  19433.,  19272.,
        19150.,  19075.,  19076.,  18762.,  19114.,  24600.,  24351.,
        24563.,  24455.,  24332.,  24048.,  23989.,  24323.,  24148.,
        24070.,  23984.,  23700.,  24005.,  23771.,  23772.,  23630.,
        23799.,  23264.,  23237.,  23330.,  23182.,  23318.,  23163.,
        23182.,  22866.,  22943.,  22758.,  22758.,  23209.,  22651.,
        22685.,  22414.,  22410.,  22564.,  22448.,  22556.,  22437.,
        22247.,  22000.,  22318.,  21825.,  21911.,  21756.,  21643.,
        21957.,  21577.,  21441.,  21683.,  21774.,  21188.]
...
for a_file in event_file_names:
    ...
    isTrack = np.array(file_labels[:,8],dtype=int)  
    ...
    if flatten == True:
        ...
        for index,e in enumerate(energy):
            ...
            if isTrack[index] == 1:
                ...
                print("Not track or cascade, skipping...")
                continue
    ...
    print("Total tracks saved: %i, Total cascades saved: %i, Saved this file: %i, Finsihed file: %s"%(sum(count_energy_track),sum(count_energy_cascade),sum(keep_index),a_file))
    ...
    if type(max_events_per_bin) is int:
        if np.all(count_energy_track >= max_events_per_bin) and np.all(count_energy_cascade >= max_events_per_bin):
            print("All bins filled, quitting...")
            break
        else:
            if verbose:
                print(count_energy_track)
                print(count_energy_cascade)
