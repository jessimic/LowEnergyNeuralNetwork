...
mask_numu = (Y_labels[:,9]) == 14
mask_nue = (Y_labels[:,9]) == 12
mask_nutau = (Y_labels[:,9]) == 16
mask_muon = (Y_labels[:,9]) == 13
print(sum(mask_numu),sum(mask_nue),sum(mask_muon),sum(mask_nutau))
weights = np.ones(Y_labels.shape[0])
...
def plot_output(Y_values,outdir,filenumber=None,names=output_names,transform=output_factors,weights=None):
    ...
    emax = 500
    plt.figure()
    plt.hist(Y_values[:,0][mask_numu]*transform[0],bins=100,range=[0,emax],weights=weights[mask_numu],label="NuMu",alpha=0.5);
    plt.hist(Y_values[:,0][mask_nue]*transform[0],bins=100,range=[0,emax],weights=weights[mask_nue],label="NuE",alpha=0.5);
    plt.hist(Y_values[:,0][mask_muon]*transform[0],bins=100,range=[0,emax],weights=weights[mask_muon],label="Muon",alpha=0.5);
    ...
    emax = 500
    emin = 0
    values = Y_values[:,1]*transform[1]
    plt.hist(values[mask_muon],range=[emin,emax],bins=100,weights=weights[mask_muon],label=r'$\mu$',alpha=0.5);
    plt.hist(values[mask_numu],range=[emin,emax],bins=100,weights=weights[mask_numu],label=r'$\nu_\mu$',alpha=0.5);
    plt.hist(values[mask_nue],range=[emin,emax],bins=100,weights=weights[mask_nue],label=r'$\nu_e$',alpha=0.5);
    #plt.hist(values[mask_tau],range=[emin,emax],bins=100,weights=weights[mask_tau],label=r'$\nu_\tau$',alpha=0.5);
    ...
    plt.figure()
    plt.hist(values[mask_numu],bins=100,weights=weights[mask_numu],label="NuMu",alpha=0.5);
    plt.hist(values[mask_nue],bins=100,weights=weights[mask_nue],label="NuE",alpha=0.5);
    plt.hist(values[mask_muon],bins=100,weights=weights[mask_muon],label="Muon",alpha=0.5);
    ...
    if Y_values.shape[-1] == 13:
        plt.figure()
        plt.hist(Y_values[:,12][mask_numu],bins=100,weights=weights[mask_numu],label="NuMu",alpha=0.5);
        plt.hist(Y_values[:,12][mask_nue],bins=100,weights=weights[mask_nue],label="NuE",alpha=0.5);
        plt.hist(Y_values[:,12][mask_muon],bins=100,weights=weights[mask_muon],label="Muon",alpha=0.5);
    ...
    for i in range(0,len(units)):
        if i == 8:
            ...
            a_name = names[i]
        ax[row_index, col_index].hist(values[mask_muon],bins=100,weights=weights[mask_muon],label=r'$\mu$',alpha=0.5);
        ax[row_index, col_index].hist(values[mask_numu],bins=100,weights=weights[mask_numu],label=r'$\nu_\mu$',alpha=0.5);
        ax[row_index, col_index].hist(values[mask_nue],bins=100,weights=weights[mask_nue],label=r'$\nu_e$',alpha=0.5);
...
if do_output:
    plot_output(Y_labels,outdir,filenumber=filenum,names=output_names,weights=weights) #weights=weights_test[:,8]/1510.
