...
def get_observable_features(frame):
    ...
    array_DC = numpy.zeros([len(DC_strings),60,9])
    count_outside = 0
    charge_outside = 0
    ...
    for omkey, pulselist in ice_pulses:
        ...
        if DC_flag == True:
            ...
            array_DC[string_index,dom_index,3] = time_array[0]
            array_DC[string_index,dom_index,4] = time_array[time_20p]
            array_DC[string_index,dom_index,5] = time_array[time_50p]
            array_DC[string_index,dom_index,6] = time_array[-1]
            array_DC[string_index,dom_index,7] = weighted_avg_time
            array_DC[string_index,dom_index,8] = weighted_std_time
...
def read_files(filename_list, drop_fraction_of_tracks=0.88, drop_fraction_of_cascades=0.00):
    ...
    for event_file_name in filename_list:
        ...
        while event_file.more():
            ...
            DC_array  = get_observable_features(frame)
    ...
    return output_features_DC, output_labels
...
features_DC, labels = read_files(event_file_names, drop_fraction_of_tracks=0.0,drop_fraction_of_cascades=1.0)
