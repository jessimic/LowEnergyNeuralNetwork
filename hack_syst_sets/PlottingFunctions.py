...
def plot_history_from_list(loss,val,save=False,savefolder=None,logscale=False,ymin=None,ymax=None,title=None,variable="Energy",pick_epoch=None,lr_start=None,lr_drop=None,lr_epoch=None,step=1,notebook=False,xline=None,xline_label="Changed Model"):
    ...
    if variable=="Energy":
        plt.ylabel(r'Loss = $\frac{100}{n}\sum_{i=1}^n \vert \frac{T_i - R_i}{T_i} \vert$',fontsize=20) #mean absolute percentage error
    elif variable=="Cosine Zenith" or variable=="Vertex":
        plt.ylabel(r'Loss = $\frac{1}{n}\sum_{i=1}^n ( T_i - R_i )^2$',fontsize=20) #mean squared error
    elif variable=="Muon" or variable=="Muon Classification" or variable=="PID" or variable=="Track" or variable=="Track Classification":
        plt.ylabel(r'Loss = $\frac{1}{n}\sum_{i=1}^n T_i \log(R_i) + (1+T_i)\log(1-R_i) $',fontsize=20) #Binary cross entropy
    else:
