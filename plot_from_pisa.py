...
#Calculated variables
    ...
    if cnn_file:
        mask[a_set]['ProbTrack'] = reco[a_set]['prob_track'] >= 0.55
    else:
        mask[a_set]['ProbTrack'] = reco[a_set]['prob_track'] >= 0.85
...
############## CHANGE THESE LINES ##############
variable_index_list = [1] #[1,2,3] #[0] #[1,2,3] #chose variable from list above
...
if print_cuts:
    ...
    if sum(sample_mask1) > 0:
        ...
        check_cuts = ["RecoNoR", "RecoNoZ", "RecoNoEn", "RecoNoZenith", "RecoNoVer"]
        for a_cut in check_cuts:
...
#NEED BINARY PROBMU
    for a_set in set_names:
        percent_save = my_confusion_matrix(true[a_set]['isNu'],
                    mask[a_set]['ProbMu'], weights[a_set],
                    mask=mask[a_set]['Analysis'],
...
if plot_muon_eff:
    ...
    title_here="Muon Classifier Efficiency"
    ...
    for a_set in set_names:
        efficiency_mu[a_set] = {}
        cut_variable_here = mask[a_set]['ProbMu']
...
if plot_pid_eff:
    ...
    for a_set in set_names:
        ...
        #reco_cut_minus_var = true[a_set]["All"] #mask[a_set]["RecoNoMuCut"]
        analysis_cut = mask[a_set]["Analysis"] #np.logical_and(reco_cut_minus_var, mask[a_set]["MC"])
...
#NEED BINARY PROB TRACK
    for a_set in set_names:
        mask_here = np.logical_and(mask1['Analysis'], true1['isNu'])
        percent_save = my_confusion_matrix(true[a_set]['isTrack'], 
                    true[a_set]['prob_track'],
                    weights[a_set],label0="Cascade",label1="Track",
                    mask=mask_here,title="%s PID Cut"%name_dict[a_set],
                    save=save,save_folder_name=save_folder_name)
...
if make_test_samples:
    ...
    energy_here = true[set1]['energy']
    weights_here = weights[set1]
    plt.hist([true[set1]['energy'][selection_numucc],true[set1]['energy'][selection_nuecc]], bins=bins,color=[color[2],color[1]],range=[minval_here,maxval_here],weights=[weights_here[selection_numucc],weights_here[selection_nuecc]],label=[pretty_flavor_name[1],pretty_flavor_name[0]],stacked=True);
    plt.ylabel("Rate (Hz)")
    ...
    selection_numucc = np.logical_and(true[set1]['isNuMu'], true[set1]['isCC'])
    selection_nuecc = np.logical_and(true[set1]['isNuE'], true[set1]['isCC'])
    plt.hist([true[set1]['coszenith'][selection_numucc],true[set1]['coszenith'][selection_nuecc]], bins=bins,color=[color[2],color[1]],range=[minval_here,maxval_here],weights=[weights[set1][selection_numucc],weights[set1][selection_nuecc]],label=[pretty_flavor_name[1],pretty_flavor_name[0]],stacked=True);
    plt.ylabel("Rate (Hz)")
