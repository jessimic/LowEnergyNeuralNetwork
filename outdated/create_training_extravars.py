##############################
# NOTE: Code sums rings on IceCube into a single input "DOM"
# Not integrated into latest i3_to_hdf5.py changes
# Need to merge the two codes if using
# Changes by Emma Hettinger
#############################
...
def get_observable_features(frame,low_window=-500,high_window=4000):
    ...
    array_DC = numpy.zeros([len(DC_strings),60,9]) #9 #take only DOMs in main region, not veto layer
    array_IC_near_DC = numpy.zeros([len(IC_near_DC_strings),60,9]) #9
    ...
    for omkey, pulselist in ice_pulses:
        ...
        if DC_flag == True or IC_near_DC_flag == True:
            ...
            print("charge shape:",charge_array.shape,"time shape:",time_array.shape)
            ...
            print("time shifted shape:",time_shifted.shape)
            ...
            # things are getting removed after creating mask!
            # prob a fix in more recent version of code
            ...
            time_shifted = numpy.array(time_shifted)
            print("time shifted shape:",time_shifted.shape)
            mask_500 = time_array<500 # be careful about time array vs time shifted here - or just recalculate
            mask_100 = time_shifted<100
            ...
            sum_charge = numpy.cumsum(charge_array) # changed from chargelist!!
            flag_20p = False
        ...
        if DC_flag == True:
            ...
            array_DC[string_index,dom_index,5] = sum(charge_array[mask_500])
            array_DC[string_index,dom_index,6] = sum(charge_array[mask_100])
            array_DC[string_index,dom_index,7] = time_array[time_20p]
            array_DC[string_index,dom_index,8] = time_array[time_50p]
        ...
        if IC_near_DC_flag == True:
            ...
            print("charge array shape:",charge_array.shape,"mask 500 shape",mask_500.shape)
            array_IC_near_DC[string_index,dom_index,5] = sum(charge_array[mask_500])
            array_IC_near_DC[string_index,dom_index,6] = sum(charge_array[mask_100])
            array_IC_near_DC[string_index,dom_index,7] = time_array[time_20p]
            array_IC_near_DC[string_index,dom_index,8] = time_array[time_50p]
...
output_path = "/mnt/scratch/hettin20/training_files/" + output_name + "_lt" + str(int(emax)) + "_IC" + str(ICstrings) + "_9vars"+ ".hdf5"
